...
final class FLBuilderAdminPosts {
	...
	/**
	 * Renders custom column headings for builder templates.
	 *
	 * @since 1.7
	 * @param array $columns An array of existing post columns.
	 * @return array
	 */
	static public function render_templates_post_columns( $columns )
	{
		global $post;
	...
	/**
	 * Renders custom column content for builder templates.
	 *
	 * @since 1.7
	 * @param string $column The column slug.
	 * @return void
	 */
	static public function render_templates_post_columns_content( $column )
	{
		global $post;
		
		if ( 'fl-builder-template-shortcode' == $column ) {
			//echo '<input type="text" class="text" value=\'[fl_builder_insert_layout id="' . $post->ID . '"]\' readonly="readonly" onClick="this.setSelectionRange(0, this.value.length)" style="font-size:13px; max-width:240px; width: 100%;" />';
		}
	}
